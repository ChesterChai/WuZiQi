...
public class GameMap {
    ...
    private  static long[][] weightBlack;
    private  static long[][] weightWhite;
    boolean weight_initialized = false;

    GameMap(Color[][] map) {
        this.map = map;

        if(map.length != Config.size || !weight_initialized) initWeight();
    ...
    private static void initWeight(){
        ...
        weightBlack =  new long[Config.size][Config.size];
        weightWhite = new long[Config.size][Config.size];
        for (int i = 0; i < Config.size; i++)
            for (int j = 0; j < Config.size; j++) {
